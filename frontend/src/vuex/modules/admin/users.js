...
const actions = {
  async loadUsers({ commit }) {
    try {
      const r = await api.get(`admin/inventories/${this.$currentInventoryId}/users`);
      commit('SET_USERS', r);
    } catch (e) {}
  },
  ...
  async createUser({ commit, dispatch }, { payload }) {
    try {
      const r = await api.post(`admin/inventories/${this.$currentInventoryId}/users`, payload);
  ...
  async updateUser({ commit, dispatch }, { id, payload }) {
    try {
      const r = await api.update(`admin/inventories/${this.$currentInventoryId}/users/${id}`, payload);
  ...
  async destroyUser({ commit, dispatch }, { id }) {
    try {
      const r = await api.delete(`admin/inventories/${this.$currentInventoryId}/users/${id}`);
